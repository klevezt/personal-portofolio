...
function IndexCarousel() {
  return (
    <Box className={styles["kp-index-slider-wrapper"]}>
      <div className="absolute top-32 left-1/2 -translate-x-1/2 h-auto text-white font-custom ">
        <h1 className="text-6vw">Make it work.</h1>
        <h1 className="text-6vw">Make it right.</h1>
        <h1 className="text-6vw">Make it fast.</h1>
      </div>

      <div className="absolute bottom-0 left-1/4 right-1/4 h-auto text-white">
        <Carousel
          controls={false}
          indicators={false}
          interval={7000}
          ...
          {images.map((step, index) => (
            <Carousel.Item key={index} className="min-h-[300px]">
              <div className="flex justify-center">
                ...
                />
                <div className="border-l-2 ml-8 p-8">
                  <h4>{step.label}</h4>
